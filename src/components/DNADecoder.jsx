...
function DNADecoder() {
    ...
    useEffect(() => {
        // Load the DNA key (assuming 'avatar' type for now)
        import(`../data/DNA_keys/avatar_DNA_key_v1.json`)
            .then((key) => setDnaKey(key.default))
    ...
    return (
        <div>
            ...
            {decodedData && <Typography variant="body1" style={{ marginTop: '20px' }}>Decoded Data: <pre>{decodedData}</pre></Typography>}
        </div>
    );
}
